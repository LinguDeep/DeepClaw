...
const DEEPCLAW_DIR = process.env.DEEPCLAW_DIR || path.join(os.homedir(), '.deepclaw');
const WORKSPACE_DIR = process.env.WORKSPACE_DIR || process.env.DEEPCLAW_WORKSPACE || process.cwd();
const AGENT_ID = process.env.DEEPCLAW_AGENT || 'main';
const ENGINE_BRIDGE_HOST = process.env.ENGINE_BRIDGE_HOST || '127.0.0.1';
const ENGINE_BRIDGE_PORT = parseInt(process.env.ENGINE_BRIDGE_PORT || '9000');
const sessDir = path.join(DEEPCLAW_DIR, 'agents', AGENT_ID, 'sessions');
const cronFile = path.join(DEEPCLAW_DIR, 'cron', 'jobs.json');
...
const configFiles = [
  { name: 'deepclaw-gateway.service', path: path.join(os.homedir(), '.config/systemd/user/deepclaw-gateway.service') },
  { name: 'deepclaw-config.json',     path: path.join(os.homedir(), '.deepclaw/config.json') },
];
...
const READ_ONLY_FILES = new Set(['deepclaw-gateway.service', 'deepclaw-config.json']);
  ...
  try {
    ...
    if (process.platform === 'linux') {
      try {
        ...
        let logs = '';
        try {
          logs = execSync("journalctl -u deepclaw --since '7 days ago' --no-pager -o short 2>/dev/null | grep -ci 'SIGABRT\\|SIGSEGV\\|exit code [1-9]\\|process crashed\\|fatal error' || echo 0", { encoding: 'utf8' }).trim();
        } catch {
          // If system fails, try user scope
          logs = execSync("journalctl --user -u deepclaw --since '7 days ago' --no-pager -o short 2>/dev/null | grep -ci 'SIGABRT\\|SIGSEGV\\|exit code [1-9]\\|process crashed\\|fatal error' || echo 0", { encoding: 'utf8' }).trim();
        }
      ...
      try {
        ...
        let logs = '';
        try {
          logs = execSync("journalctl -u deepclaw --since today --no-pager -o short 2>/dev/null | grep -ci 'SIGABRT\\|SIGSEGV\\|exit code [1-9]\\|process crashed\\|fatal error' || echo 0", { encoding: 'utf8' }).trim();
        } catch {
          // If system fails, try user scope
          logs = execSync("journalctl --user -u deepclaw --since today --no-pager -o short 2>/dev/null | grep -ci 'SIGABRT\\|SIGSEGV\\|exit code [1-9]\\|process crashed\\|fatal error' || echo 0", { encoding: 'utf8' }).trim();
        }
...
function getServicesStatus() {
  const { execSync } = require('child_process');
  const services = ['deepclaw', 'agent-dashboard', 'tailscaled'];
...
// WebSocket Bridge Client for Rust Engine
let engineBridge = null;
const terminalLogClients = new Set();

function connectToEngineBridge() {
  const bridgeUrl = `ws://${ENGINE_BRIDGE_HOST}:${ENGINE_BRIDGE_PORT}`;
  console.log(`[DeepClaw] Connecting to engine bridge at ${bridgeUrl}`);

  try {
    engineBridge = new WebSocket(bridgeUrl);

    engineBridge.on('open', () => {
      console.log('[DeepClaw] Connected to Rust engine bridge');
    });

    engineBridge.on('message', (data) => {
      const message = JSON.parse(data.toString());
      // Broadcast terminal logs to connected dashboard clients
      if (message.type === 'BashResult' || message.type === 'FileContent') {
        terminalLogClients.forEach(client => {
    ...
    engineBridge.on('error', (error) => {
      console.error('[DeepClaw] Engine bridge error:', error.message);
    });

    engineBridge.on('close', () => {
      console.log('[DeepClaw] Engine bridge disconnected, reconnecting in 5s...');
      setTimeout(connectToEngineBridge, 5000);
    });
  } catch (error) {
    console.error('[DeepClaw] Failed to connect to engine bridge:', error.message);
    setTimeout(connectToEngineBridge, 5000);
  }
}

// Connect to engine bridge on startup
connectToEngineBridge();
...
const server = http.createServer((req, res) => {
  ...
  if (req.url.startsWith('/api/')) {
    ...
    // Engine bridge endpoints (no auth required for internal use)
    if (handleEngineBridgeRequest(req, res)) return;
...
// WebSocket Server for Terminal Log Streaming
const wss = new WebSocket.Server({ noServer: true });

server.on('upgrade', (request, socket, head) => {
  const pathname = new URL(request.url, `http://${request.headers.host}`).pathname;

  if (pathname === '/terminal-logs') {
...
wss.on('connection', (ws) => {
  console.log('[DeepClaw] Dashboard client connected for terminal logs');
  terminalLogClients.add(ws);

  ws.on('close', () => {
    console.log('[DeepClaw] Dashboard client disconnected from terminal logs');
    terminalLogClients.delete(ws);
  });
});

// API endpoint: Execute bash command via engine bridge
function handleEngineBridgeRequest(req, res) {
  if (req.method === 'POST' && req.url === '/api/engine/bash') {
    let body = '';
    req.on('data', chunk => { body += chunk; });
    req.on('end', () => {
      try {
        const { command, cwd } = JSON.parse(body);
        if (engineBridge && engineBridge.readyState === WebSocket.OPEN) {
          ...
          const responseHandler = (data) => {
            ...
            if (message.id === requestId) {
              engineBridge.off('message', responseHandler);
              res.writeHead(200, { 'Content-Type': 'application/json' });
              res.end(JSON.stringify(message));
            }
          };

          engineBridge.on('message', responseHandler);
          engineBridge.send(JSON.stringify(request));
          ...
          res.writeHead(503);
          res.end(JSON.stringify({ error: 'Engine bridge not connected' }));
        }
      } catch (error) {
        res.writeHead(400);
  ...
  if (req.method === 'GET' && req.url.startsWith('/api/engine/read')) {
...
// Inject engine bridge handler into the main request handler
const originalCreateServer = server;
// Note: The handler is already integrated above, this is just for reference

server.listen(PORT, '0.0.0.0', () => {
  console.log('Dashboard: http://0.0.0.0:' + PORT);
  console.log('Terminal Logs WebSocket: ws://0.0.0.0:' + PORT + '/terminal-logs');
});
