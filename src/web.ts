...
import { getIntegrationRegistry } from './api-integrations';
...
export class WebUIManager {
  ...
  private setupRoutes(): void {
    ...
    codeSandbox.init().catch(() => {});

    this.app.post('/api/sandbox/execute', async (req: Request, res: Response) => {
      const { language, code, timeout, packages } = req.body;
      if (!language || !code) { res.status(400).json({ error: 'language and code required' }); return; }
      try {
        const result = await codeSandbox.execute({ language, code, timeout, packages });
        res.json(result);
      } catch (e: any) { res.status(500).json({ error: e.message }); }
    });

    this.app.get('/api/sandbox/languages', async (_req: Request, res: Response) => {
      try {
        const languages = await codeSandbox.getAvailableLanguages();
        res.json({ languages, dockerAvailable: codeSandbox.isDockerAvailable });
      } catch (e: any) { res.status(500).json({ error: e.message }); }
    });

    // ============ CHAIN OF THOUGHT API ============
    const cot = getChainOfThought();

    this.app.get('/api/cot/trace', (_req: Request, res: Response) => {
      res.json({ trace: cot.getReadableTrace(), chain: cot.getChain() });
    });

    this.app.get('/api/cot/thoughts', (req: Request, res: Response) => {
      const type = req.query.type as string | undefined;
      res.json(cot.getThoughts(undefined, type as any));
    });

    this.app.get('/api/cot/actions', (req: Request, res: Response) => {
      const tool = req.query.tool as string | undefined;
      res.json(cot.getActions(undefined, tool));
    });

    this.app.get('/api/cot/summary', (_req: Request, res: Response) => {
      const summary = cot.getSessionSummary();
      res.json(summary || { error: 'No active session' });
    });

    // ============ SESSION MEMORY API ============
    const sessionMem = getSessionMemory();

    this.app.get('/api/session/stats', (_req: Request, res: Response) => {
      res.json(sessionMem.getStats());
    });

    this.app.get('/api/session/context', (_req: Request, res: Response) => {
      res.json({ context: sessionMem.getContextForLLM() });
    });

    this.app.get('/api/session/history', (_req: Request, res: Response) => {
      res.json(sessionMem.getActionHistory());
    });

    // ============ API INTEGRATIONS ============
    const integrationRegistry = getIntegrationRegistry();

    this.app.get('/api/integrations', (_req: Request, res: Response) => {
      res.json({ registered: integrationRegistry.list() });
    });

    this.app.post('/api/integrations/register', (req: Request, res: Response) => {
      const { name, type, token } = req.body;
      if (!name || !type || !token) { res.status(400).json({ error: 'name, type, and token required' }); return; }
      try {
        if (type === 'github') {
          integrationRegistry.registerGitHub(token);
        } else if (type === 'slack') {
          integrationRegistry.registerSlack(token);
        } else {
          integrationRegistry.registerAPI(name, {
            name,
            baseUrl: req.body.baseUrl || '',
            credentials: { type: 'bearer', token },
          });
        }
        res.json({ success: true, registered: integrationRegistry.list() });
      } catch (e: any) { res.status(500).json({ error: e.message }); }
    });
    ...
    this.app.get('/api/system/status', (_req: Request, res: Response) => {
      ...
      res.json({
        ...
        codeSandbox: { dockerAvailable: codeSandbox.isDockerAvailable },
        chainOfThought: { activeSession: cot.getActiveSessionId() },
        integrations: { registered: integrationRegistry.list() },
      });
    });
...
Provide a brief summary and suggested reply.` }
  ...
  /**
   * Setup WebSocket for real-time updates including Chain of Thought streaming
   */
  ...
  private handleWebSocketMessage(ws: WebSocket, message: any): void {
    if (message.type === 'ping') {
    ...
    } else if (message.type === 'get_reasoning_trace') {
      // Return the full reasoning trace for current session
      if (this.orchestrator) {
        ws.send(JSON.stringify({
          type: 'reasoning_trace',
          payload: { trace: this.orchestrator.getReasoningTrace() },
        }));
      }
      ...
      if (this.orchestrator) {
        ws.send(JSON.stringify({
          type: 'session_stats',
          payload: this.orchestrator.getSessionStats(),
        }));
      }
    }
  }
  ...
  /**
   * Subscribe to Chain of Thought events and broadcast to WebSocket clients
   */
  private setupCoTStreaming(orchestrator: Orchestrator): void {
    const cot = orchestrator.cot;

    cot.on('thought', (thought: any) => {
      this.broadcast({
        type: 'thought',
        payload: thought,
        timestamp: new Date(),
    ...
    cot.on('action:start', (action: any) => {
      this.broadcast({
        type: 'action_start',
        payload: action,
        timestamp: new Date(),
    ...
    cot.on('action:update', (action: any) => {
      this.broadcast({
        type: 'action_update',
        payload: action,
        timestamp: new Date(),
    ...
    cot.on('session:end', (summary: any) => {
      this.broadcast({
        type: 'session_end',
        payload: summary,
        timestamp: new Date(),
    ...
    // TaskPlanner events
    orchestrator.taskPlanner.on('planning:start', (data: any) => {
      this.broadcast({ type: 'planning_start', payload: data, timestamp: new Date() });
    });

    orchestrator.taskPlanner.on('planning:complete', (data: any) => {
      this.broadcast({ type: 'planning_complete', payload: data, timestamp: new Date() });
    });

    orchestrator.taskPlanner.on('subtask:start', (data: any) => {
      this.broadcast({ type: 'subtask_start', payload: data, timestamp: new Date() });
    });

    orchestrator.taskPlanner.on('subtask:complete', (data: any) => {
      this.broadcast({ type: 'subtask_complete', payload: data, timestamp: new Date() });
    });

    orchestrator.taskPlanner.on('subtask:fail', (data: any) => {
      this.broadcast({ type: 'subtask_fail', payload: data, timestamp: new Date() });
    });
  }

  /**
   * Run task and broadcast updates with Chain of Thought streaming
   */
  private async runTask(task: string): Promise<void> {
    if (!this.orchestrator) return;

    // Setup real-time CoT streaming for this orchestrator
    this.setupCoTStreaming(this.orchestrator);

    // Initialize subsystems
    await this.orchestrator.initSystems();
    ...
    try {
      ...
      this.broadcast({
        ...
        payload: {
          result,
          reasoningTrace: this.orchestrator.getReasoningTrace(),
          stats: this.orchestrator.getSessionStats(),
        },
      });
    } catch (error: any) {
      this.broadcast({
